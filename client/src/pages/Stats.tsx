...
export default function Stats() {
  ...
  // Handle unauthenticated users via useEffect to avoid render-time side effects
  useEffect(() => {
    if (!isAuthenticated) {
      setLocation("/");
    }
  ...
  const firstHalfData = anxietyData.slice(0, halfPoint);
  const secondHalfData = anxietyData.slice(halfPoint);
  const firstHalfAvg = firstHalfData.length > 0
    ? firstHalfData.reduce((sum, e) => sum + e.anxietyBefore, 0) / firstHalfData.length
    : 0;
  const secondHalfAvg = secondHalfData.length > 0
    ? secondHalfData.reduce((sum, e) => sum + e.anxietyBefore, 0) / secondHalfData.length
    : 0;
