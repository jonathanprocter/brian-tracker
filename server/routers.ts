  ...
  entries: router({
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Update streak - normalize dates to midnight for consistent comparison
        ...
        if (lastCompletion) {
          ...
          const lastCompletionMidnight = new Date(lastCompletion);
          lastCompletionMidnight.setHours(0, 0, 0, 0);

          const daysSinceLastCompletion = Math.floor(
            (todayMidnight.getTime() - lastCompletionMidnight.getTime()) / (1000 * 60 * 60 * 24)
          );
